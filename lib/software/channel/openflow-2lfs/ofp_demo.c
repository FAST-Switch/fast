...
//###############################################    table         manager  ####################################3
void 	add_flow(struct  _flow_table  flow_table[],u8  *data,char * key,u32  inport,u32  outport,u32 datalength)
{
	printf("\n########################add_flow############################\n");
	...
	{
		if(strcmp(flow_table[i].key,key)!=0)
		{
			if(flow_table[i].status==0)
			{
				flow_table[i].status=1;
				flow_table[i].key=key;
				flow_table[i].in_port=inport;
				flow_table[i].out_port=outport;
				flow_table[i].data_len=datalength;
				flow_table[i].data=data;
			}
		}
	}
}
...
void   del_a_flow(char *key)
{
	...
	for( i=0;i<FLOW_NUMBER;i++)
	{
		if(strcmp(flow_table[i].key,key)==0)
		{
			//FAST=handle_match_field(flow_table[i].data,0xff);
			//status = delete(FAST);
			//if(status==1)
			//{
				flow_table[i].status=0;
				flow_table[i].key=NULL;
				flow_table[i].in_port=0;
				flow_table[i].out_port=0;
				flow_table[i].data_len=0;
				flow_table[i].data=NULL;
			//}
	...
	{	
		if(flow_table[count].status>0)
		{
			if(flow_table[count].in_port==port||flow_table[count].out_port==port)
			{
				...
				if(status==1)
				{
				flow_table[count].status=0;
				flow_table[count].key=NULL;
		...
		case 0:
			{
				...
				if(ntohs(inst->len)>8)
				{
					...
					MD5Final(&md5,hash_value);
					char * key=(char *)malloc(16);
					char *key_tmp=(char *)malloc(1);
					int j;
					for( j=0;j<16;j++)
					{
						sprintf(key_tmp,"%02x",hash_value[j]);
						strcat(key,key_tmp);
					}
					free(key_tmp);
					printf("key:%s\n",key);
		...
		case 3:
		#if 0
			{
				...
				if(match_len>4)
				{
					...
					//out = ( struct ofp_action_output *)&ofpbuf->data[sizeof(struct ofp_flow_mod)-sizeof(struct ofp_match)+match_len+pad+8];//sizeof(struct ofp_instruction
					//printf("output:%d\n",ntohl(out->port));
					match = malloc(match_len+pad+ntohs(inst->len));
					memset(match,0,match_len+pad+ntohs(inst->len));
					memcpy((u8*)match,(u8*)&ofpbuf->data[sizeof(struct ofp_flow_mod)-sizeof(struct ofp_match)],match_len+pad+ntohs(inst->len));
					MD5Init(&md5);
					MD5Update(&md5,(unsigned char *)match,match_len+pad+ntohs(inst->len));
					MD5Final(&md5,hash_value);
					char * key=(char *)malloc(16);
					char *key_tmp=(char *)malloc(1);
					for(int i=0;i<16;i++)
					{
						sprintf(key_tmp,"%02x",hash_value[i])
						strcat(key,key_tmp)
					}
					free(key_tmp);
					printf("\n######################## delete a flow key:%s########################\n",key);
...
//	del_flow_by_port(3);
	...
	for(i=0;i<FLOW_NUMBER;i++)
	{
		printf("flow_table[%d].status:%d\n",i,flow_table[i].status);
		if(flow_table[i].status>0)
		{
