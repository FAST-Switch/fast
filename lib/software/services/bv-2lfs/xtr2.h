...
u_int16_t * get_mask(char ** a); //get rule mask
u_int16_t * get_new_a (char ** a ); //delete '*';
...
void struct_to_char( struct sw_flow *sw_0);
int * delete_record;
u_int8_t record;


struct fast_table{
	struct sw_flow sw_flow_key;
	struct sw_flow sw_flow_mask;
}fast;
struct fast_table ft[36];
int rule_count;
...
char * rule_char;
char * sub;
void write_actions(struct fast_table *fast,int addr);
...
int change_col;
int real_num;
int actions_addr;// write_addr_action;
int *action_record;
int delete_call;
/********************************BV**********************************/
