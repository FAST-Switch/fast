...
if {[expr $IS_SMALLMAC == 1]} {
   ...
   if {[expr ($STAT_CNT_ENA == 1)]} {
      # Clock Crosser (used in statistic register clock crossing)
...
if {[expr ($ENABLE_GMII_LOOPBACK == 1)]} {
   ...
   set_false_path -from [get_registers {*|altera_tse_a_fifo_24:U_LBFF|altera_tse_gray_cnt:U_RD|b_out[*]}] -to [get_registers {*|altera_tse_a_fifo_24:U_LBFF|rd_g_wptr[*]}]
